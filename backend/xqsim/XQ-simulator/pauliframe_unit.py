...
class pauliframe_unit:
    def __init__ (self, unit_stat, config):
        ...
        # ============================================================================
        # [QuStitch拡張] sticky_error_pulse レジスタ（オプトイン、デフォルトは上流と同一挙動）
        # EDUのoutput_validは1サイクルパルスで、"waiting"以外の状態で到着すると失われる。
        # 有効時はパルス(とpfflag)をラッチし、次の"waiting"で消費する。
        # ============================================================================
        self.sticky_error_pulse = False
        self.errorpend_reg = False
        self.pfflagpend_reg = False
        # ============================================================================
        # [QuStitch拡張] ここまで
        # ============================================================================
    ...
    def transfer_control(self):
        ...
        if self.state == "ready":
            ...
            # ============================================================================
            # [QuStitch拡張] sticky_error_pulse: 条件に「or (…errorpend_reg)」を追記
            # （上流は self.input_error_valid のみ）
            # ============================================================================
            if self.input_error_valid or (self.sticky_error_pulse and self.errorpend_reg):
                self.next_state = "ready"
            else:
                self.next_state = "waiting"
        else:
        ...
        # next_valid
        # ============================================================================
        # [QuStitch拡張] sticky_error_pulse: 条件に「or (…pfflagpend_reg)」を追記
        # （上流は self.input_pfflag のみ）
        # ============================================================================
        if self.state == "waiting" and \
           self.next_state == "ready" and \
           (self.input_pfflag or (self.sticky_error_pulse and self.pfflagpend_reg)):
    ...
    def update_registers(self):
        ...
        # ============================================================================
        # [QuStitch拡張] errorpend_reg / pfflagpend_reg の更新（sticky_error_pulse）
        # 消費(waiting→ready)でクリア、未消費のパルス到着でセット。
        # ============================================================================
        if self.sticky_error_pulse:
            if self.state == "waiting" and self.next_state == "ready":
                self.errorpend_reg = False
                self.pfflagpend_reg = False
            elif self.input_error_valid:
                self.errorpend_reg = True
                if self.input_pfflag:
                    self.pfflagpend_reg = True
        # ============================================================================
        # [QuStitch拡張] ここまで
        # ============================================================================
